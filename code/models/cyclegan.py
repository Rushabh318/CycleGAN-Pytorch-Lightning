...
class Generator(nn.Module):
    ...
    def __init__(self, img_sz: int = 256, in_channels: int = 3, out_channels: int = 64, apply_dp: bool = True):
        ...
        for i in range(nb_downsampling):
            conv = nn.ConvTranspose2d(out_channels * f, out_channels * (f//2), 
        ...
        for image in images:
            ...
            if  self.nb_images < self.pool_sz:
                ...
                if np.random.uniform(0, 1) > 0.5:
                    ...
                    temp_img = self.image_pool[int(rand_int)].clone()
                    self.image_pool[int(rand_int)] = image
...
class Loss:
    ...
    def get_gen_loss(self, real_A, real_B, cyc_A, cyc_B, idt_A, idt_B, 
                     d_A_pred_fake_data,
                     d_B_pred_fake_data):
...
class CycleGAN(LightningModule):
    ...
    def __init__(self, 
        ...
        self.d_A = init(Discriminator(in_channels = self.in_channels, 
                                      out_channels = self.out_channels, 
                                      nb_layers = 3))
        self.d_B = init(Discriminator(in_channels = self.in_channels, 
                                      out_channels = self.out_channels, 
                                      nb_layers = 3))
        self.g_A2B = init(Generator(img_sz=self.img_sz, 
                                    in_channels = self.in_channels, 
                                    out_channels = self.out_channels, 
                                    apply_dp = False))
        self.g_B2A = init(Generator(img_sz=self.img_sz, 
                                    in_channels = self.in_channels, 
                                    out_channels = self.out_channels, 
                                    apply_dp = False))
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            g_A2B_loss, g_B2A_loss, g_tot_loss = self.loss.get_gen_loss(real_A, real_B, 
                                                                        cyc_A, cyc_B, 
                                                                        idt_A, idt_B, 
    ...
    def shared_step(self, batch, stage: str = 'val'):
        ...
        dict_ = {f'g_tot_{stage}_loss': g_tot_loss, 
                 f'g_A2B_{stage}_loss': g_A2B_loss, f'g_B2A_{stage}_loss': g_B2A_loss, 
